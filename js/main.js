...
WebFont.load({
    google: {
        families: ['Montserrat:300,400,700,800:cyrillic']
    }
});
...
$('.link--seo').on('click',  function(event) {
	...
	height_element = $('.paragraph--seo').height()
	// $('.seo-text').addClass('active')
	$('.seo-text').animate({height:height_element+'px'},200, function() {
 ...
 $('.faq-item__head').on('click', function(event) {
        ...
        $contentThis.find('p').addClass('animated fadeIn');
        $contentThis.slideToggle()
    });
   
  $('.btn--faq').on('click', function(event) {
  	event.preventDefault();
  	$(this).addClass('animated faster fadeOutDown no')
  	$('.faq-item').slideDown();
  	/* Act on the event */
  });
  $('select').styler({
